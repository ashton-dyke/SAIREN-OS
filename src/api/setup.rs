...
pub async fn save_config(
    ...
    let toml_content =
        format!("# SAIREN-OS Well Configuration\n# Generated by setup wizard\n\n{toml_body}");
...
fn update_env_var(contents: &str, key: &str, value: &str) -> String {
    // Sanitize: strip newlines to prevent env var injection, quote the value
    let sanitized = value.replace('\n', "").replace('\r', "");
    let quoted = format!("{}=\"{}\"", key, sanitized.replace('"', "\\\""));
    let mut found = false;
