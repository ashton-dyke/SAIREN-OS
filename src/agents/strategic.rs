...
impl StrategicAgent {
    ...
    fn verify_well_control(
        ...
        // Uncertain - need more data; let CfC break the tie
        let uncertain = (
        ...
        self.cfc_tiebreak(ticket, uncertain)
    }
    ...
    fn cfc_tiebreak(
        &self,
        ticket: &mut AdvisoryTicket,
        uncertain_result: (VerificationStatus, String, FinalSeverity, bool),
    ) -> (VerificationStatus, String, FinalSeverity, bool) {
        let score = match ticket.cfc_anomaly_score {
            Some(s) => s,
