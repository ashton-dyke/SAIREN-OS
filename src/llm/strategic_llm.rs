...
impl StrategicLLM {
    ...
    fn build_drilling_advisory_prompt(
        ...
        // Build CfC neural network section from ticket fields
        let cfc_section = match &ticket.cfc_anomaly_score {
            Some(score) => {
                let surprises_str = if ticket.cfc_feature_surprises.is_empty() {
                    String::new()
                } else {
                    let items: Vec<String> = ticket.cfc_feature_surprises.iter()
                        .take(5)
                        .map(|s| {
                            let dir = if s.error > 0.0 { "above" } else { "below" };
                            format!("  - {} ({} predicted, {:.2}\u{03c3})", s.name, dir, s.magnitude)
                        })
                        .collect();
                    format!("\nSurprising Features:\n{}", items.join("\n"))
                };
                format!("\n### CfC NEURAL NETWORK\nAnomaly Score: {:.2}/1.00 | Health: {:.2}/1.00{}",
                    score, 1.0 - score, surprises_str)
            }
            None => String::new(),
        };
        ...
        base_prompt
            ...
            .replace("{tactical_context}", &format!("{}{}", context_section, cfc_section))
    }
